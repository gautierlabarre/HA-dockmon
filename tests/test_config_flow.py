...
async def test_successful_setup_creates_the_entry(hass, mock_api):
    ...
    assert result["data"] == {CONF_URL: URL, CONF_API_KEY: "s3cret"}
...
async def test_unreachable_host_is_reported_on_the_form(hass, aioclient_mock):
    """A timeout currently falls through to the catch-all `unknown` error.

    `cannot_connect` would be the friendlier message: TimeoutError is not an
    `aiohttp.ClientError`, so the dedicated handler never sees it.
    """
